...
local queueFallbackAfterSeconds = 30
local minimumPlayersToFormLobby = 2
...
local function getOldestQueuedAt()
	local leaders = backend:GetOrderedLeaders()
	local oldestQueuedAt = nil
	for _, leaderUserId in ipairs(leaders) do
		local entry = backend:GetEntryByLeader(leaderUserId)
		if entry and type(entry.queuedAt) == "number" then
			if oldestQueuedAt == nil or entry.queuedAt < oldestQueuedAt then
...
function QueueService.Start(options)
	...
	local roundRules = GameConfigService.GetRoundRules()
	if roundRules and roundRules.Queue then
		queueFallbackAfterSeconds = math.max(0, tonumber(roundRules.Queue.FallbackAfterSeconds) or queueFallbackAfterSeconds)
		minimumPlayersToFormLobby = math.max(2, math.floor(tonumber(roundRules.Queue.MinimumEntrants) or minimumPlayersToFormLobby))
	end
...
function QueueService.GetOldestQueueAgeSeconds(now)
	local oldestQueuedAt = getOldestQueuedAt()
	if oldestQueuedAt == nil then
		return 0
	end
	return math.max(0, (now or os.clock()) - oldestQueuedAt)
end
...
function QueueService.TryFormSessions(now)
	...
	while i <= #leaders do
		local currentEntry = backend:GetEntryByLeader(leaders[i])
		if not currentEntry then
			i += 1
			continue
		end
		if #currentEntry.memberUserIds > targetLobbySize then
			backend:Cancel(currentEntry.leaderUserId)
			i += 1
			continue
		end
		...
		local selectedOldestQueuedAt = nil
		local j = i
		...
		while j <= #leaders do
			...
			if entry then
				...
				elseif selectedCount + entryCount <= targetLobbySize then
					...
					if type(entry.queuedAt) == "number" then
						if selectedOldestQueuedAt == nil or entry.queuedAt < selectedOldestQueuedAt then
							selectedOldestQueuedAt = entry.queuedAt
						end
					end
				else
					break
		...
		local selectedOldestAge = selectedOldestQueuedAt and math.max(0, now - selectedOldestQueuedAt) or 0
		local canFormAtTarget = selectedCount >= targetLobbySize
		local canFormFallback = selectedCount >= minimumPlayersToFormLobby
			and selectedCount < targetLobbySize
			and selectedOldestAge >= queueFallbackAfterSeconds
		local canForm = canFormAtTarget or canFormFallback
		if canForm then
			local memberUserIds = {}
			...
			table.insert(formed, {
				...
				oldestQueueAgeSeconds = selectedOldestAge,
				tournamentPath = tournamentPath,
...
function QueueService.BuildLobbyStateForPlayer(player)
	...
	return {
		...
		queuePopulation = queuePopulation,
		targetLobbySize = targetLobbySize,
		oldestQueueAgeSeconds = QueueService.GetOldestQueueAgeSeconds(),
		tournamentPath = tournamentPath,
